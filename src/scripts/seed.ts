...
import { blogPageData } from '@/payload/seed/data/blog'
...
const seeding = async () => {
  ...
  const formattedHomePageData = {
    ...
    blocks: homePageData?.blocks?.map(block => {
      ...
      if (block.blockType === 'Hero' || block.blockType === 'Tags') {
        return {
          ...block,
      ...
      if (block.blockType === 'LatestBlogs') {
        return {
          ...block,
  ...
  const formattedBlogPageData: Omit<Page, 'id' | 'createdAt' | 'updatedAt'> = {
    ...blogPageData,
    blocks: blogPageData?.blocks?.map(block => {
      if (block.blockType === 'BlogHero') {
        return {
          ...block,
          blogs: block.blogs?.map((blog, index) => {
            const blogId =
              blogsSeedResult.collectionsSeedingResult.at(0)?.status !==
                'skipped' &&
              blogsSeedResult.collectionsSeedingResult.at(0)?.results.at(index)
